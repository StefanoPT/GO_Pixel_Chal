...
func TestMakeMainImage(t *testing.T) {
	...
	sizeTF := len(testFile)
	expected := ExpectedValues{matrixSize: sizeTF, matrix: Matrix{Pixels: make([]byte, MatrixSize), NumberPixels: sizeTF / PixelSize, FileSize: int64(sizeTF)}}
	//Act
	err = makeMainImage(&testFile, &fn)

	//Assert

	if err != nil {
		t.Fatalf("Failed to read size of main image")
	}
